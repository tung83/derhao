...
<?php if ($_REQUEST['act']=='edit') { ?> <h3>Sửa </h3> <?php }else{ ?><h3>Thêm</h3>
</div><?php } ?>
<div class="box-body">
<form name="frm" method="post" action="index.php?com=<?=$_GET['com']?>&act=save&type=<?=$_GET['type']?>&img=<?=$_GET['img']?>&title=<?=$_GET['title']?>&desc=<?=$_GET['desc']?>&content=<?=$_GET['content']?>" enctype="multipart/form-data" class="form-horizontal " role="form" > 
	
  <input type="hidden" name="id" id="id" value="<?=@$item['id']?>" />
	<input type="submit" value="Lưu" class="btn  btn-success" />
	<input type="button" value="Thoát" onclick="javascript:window.location='index.php?com=<?=$_GET['com']?>&act=man&type=<?=$_GET['type']?>&img=<?=$_GET['img']?>&title=<?=$_GET['title']?>&desc=<?=$_GET['desc']?>&content=<?=$_GET['content']?>'" class="btn btn-warning" />
	<p></p>	
  <div class="col-xs-12">
  <div class="row">
