...
<table class="table table-bordered">
	...
	<tr bgcolor="<?php if($i%2==1)echo '#F3F3F3' ?>">
		...
		<?php } ?>
		<td align="center"><a href="index.php?com=<?=$_GET['com']?>&act=edit_danhmuc&id=<?=$items[$i]['id']?>&type=<?=$_GET['type']?>"><?=$items[$i]['ten_vi']?> 	</a> </td>
		...
		<td style="width:5%;"><a href="index.php?com=<?=$_GET['com']?>&act=edit_danhmuc&id=<?=$items[$i]['id']?>" class="icon-form"><i class="glyphicon glyphicon-pencil"></i></a></td>
		<td style="width:5%;"><a href="index.php?com=<?=$_GET['com']?>&act=delete_danhmuc&id=<?=$items[$i]['id']?>" onClick="if(!confirm('Xác nhận xóa: <?=$items[$i]['ten_vi']?>')) return false;" class="icon-form"><i class="glyphicon glyphicon-trash"></i></a></td>
	</tr>
	<?php	}?>
</table>
</div>	

<a href="index.php?com=<?=$_GET['com']?>&act=add_danhmuc&type=<?=$_GET['type']?>" class="btn btn-info btn-flat  ">Thêm mới</a>
