...
function get_main_danhmuc()
	{
		$sql="select * from table_product_danhmuc where type='".$_GET['type']."' order by stt";
...
function get_main_list()
	{
		$sql="select * from table_product_list where id_danhmuc=".@$_REQUEST['id_danhmuc']." where type='".$_GET['type']."'  order by stt";
		$stmt=mysql_query($sql);
		$str='
...
<table class=" table table-bordered">
	<tr>
		...
		<?php 
			if($_GET['class'] >=1){
				$array_danhmuc = array();
				$d->query("select ten_vi,id from #_product_danhmuc where type='".$_GET['type']."'");
		...
		<?php 
			...
			if($_GET['class'] >=2){
				$array_list = array();
				$d->query("select ten_vi,id from #_product_list  where type='".$_GET['type']."'");
				foreach($d->result_array() as $k=>$v){
	...
	<tr bgcolor="<?php if($i%2==1)echo '#F3F3F3' ?>">
		...
		<td style="width:5%;">
		<input type="checkbox" <?=($items[$i]['sale']) ? 'checked' : ''?> class="switch-input" data-type="sale" data-com="<?=$_GET['com']?>" data-id="<?=$items[$i]['id']?>"/>   
        </td>
		<td style="width:5%;">
