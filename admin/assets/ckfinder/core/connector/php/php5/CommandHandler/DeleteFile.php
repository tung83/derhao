...
class CKFinder_Connector_CommandHandler_DeleteFile extends CKFinder_Connector_CommandHandler_XmlCommandHandlerBase
{
    ...
    protected function buildXml()
    {
        ...
        if ($bDeleted) {
			...
			$_mthumbnails = $_config->getMultiConfig();
			if(is_array($_mthumbnails)){
				foreach($_mthumbnails as $k=>$v){
					$mThumb =  CKFinder_Connector_Utils_FileSystem::combinePaths($this->_currentFolder->getThumbsServerPath($v['fileName']), $fileName);
					@unlink($mThumb);
				}
			}
